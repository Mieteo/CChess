...
> Trạng thái: **P0 ✅** (2026-07-02, tắt AI đánh giá Cờ Úp + banner). **P4a Pikafish Offline ✅** (2026-07-03, §3.4). **P1 ✅ + P2 ✅** (2026-07-03, §3.5 & §6). **P3 ✅** (2026-07-03, §4.5) — còn phần nghiệm thu trên server/thiết bị thật.
...
### 4.5. ✅ P3 — ĐÃ TRIỂN KHAI (2026-07-03)

Toàn bộ §4.3 đã code xong, analyzer sạch, **Flutter test 418/418 pass** (2 skip là integration Pikafish gate env như thường lệ):

**Schema & lưu trữ:**
- `GameRecord` thêm `cupHiddenFen` (deal úp lúc khai cuộc, encode kiểu FEN-placement qua `CupRecordCodec.encodeHiddenMap` — **map tường minh, không seed**) + `cupReveals` (list song song `moves`, fenChar danh tính lật ở nước i, null nếu không lật) — cả hai nullable, tương thích ngược JSON/Hive; getter `variant` (suy từ `mode`, không lưu trùng) + `hasCupReplayData`. Firestore push kèm 2 field mới (rules cho phép, chỉ set lúc create).
- `XiangqiCupGame` giữ `initialHiddenAssignments` (map bất biến chụp lúc khởi tạo — cần vì reveal xóa dần `_hiddenAssignments`). `game_screen._persistGameResult` serialize khi ván là Cờ Úp; `game_history_repository.save()` mang field mới qua (hàm này dựng lại record từng field!).
- `CupRecordCodec.deriveReveals` là hàm thuần từ (deal ban đầu + history) nên an toàn với undo.

**Phát lại (`replay_session.dart`, export qua barrel):**
- `ReplaySession.build` chọn theo variant: `StandardReplaySession` / `CupReplaySession` (record có deal) / `LegacyCupReplaySession` (record cũ). Frame tiền tính 1 lần → seek O(1) (trước đây mỗi seek replay lại từ đầu ×2 lần).
- `CupReplaySession` phát lại bằng đúng luật `XiangqiCupGame` + deal đã lưu; mỗi frame mang `hiddenPositions` để board render quân úp; **cross-check `cupReveals`** — lệch là dừng ngay tại nước đó thay vì chiếu ván sai.
- **Dừng-đúng-nước-lỗi cho MỌI variant**: `playableMoves` = số nước áp dụng được; `ReplayUiState.atEnd`/`seek`/slider đều clamp theo nó → hết hẳn triệu chứng "highlight chạy, quân đứng im". UI hiện thẻ "Kỳ phổ lỗi từ nước N" (record hỏng) hoặc banner legacy Cờ Úp (chỉ xem move-list, bàn cờ đứng ở thế xuất phát úp toàn bộ).
- AI coach vẫn tắt cho Cờ Úp kể cả record mới có replay data (quyết định P0 — chấm điểm imperfect-information vẫn vô nghĩa).

**Test mới:** `cup_replay_session_test.dart` (codec round-trip, deriveReveals, round-trip phục bàn seed 42 so board+úp từng ply với ván gốc + `debugHiddenPieceAt`, tamper reveal log dừng đúng chỗ, legacy session, dispatch, standard dừng đúng nước lỗi) + 8 test controller/serialization trong `replay_controller_test.dart`. Lưu ý: fixture cũ trong test này chứa nước không hợp lệ mà controller cũ nuốt im lặng — đã thay bằng khai cuộc hợp lệ.

**Còn lại của P3:** record Cờ Úp legacy vĩnh viễn không phục dựng được (§4.2 — chấp nhận); chưa kiểm thử live trên thiết bị thật; ván Cờ Úp online (nếu có sau này) cần client tự ghi reveal từ server (schema đã sẵn `cupReveals`).
...
| **P3** ✅ | Schema Cờ Úp (`cupHiddenFen` + `cupReveals`) + `CupReplaySession` + xử lý record legacy + dừng-đúng-nước-lỗi cho mọi variant — xem §4.5 | — (độc lập P1/P2) | xong 2026-07-03 |
