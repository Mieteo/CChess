...
> Trạng thái: **P0 ĐÃ TRIỂN KHAI** (tắt AI đánh giá cho Cờ Úp + banner cảnh báo phục bàn Cờ Úp, 2026-07-02) — P1–P4 là kế hoạch.
...
| **P1** | Analyze server ổn định: job API (hoặc hotfix timeout), 1 search/vị trí, `evalAfterCp` + mate encoding, nhãn nguồn phân tích, cấm fallback im lặng, lưu kết quả vào record | — | BE 2-3 ngày + FE 2 ngày |
| **P2** | Biểu đồ eval + win-bar + đồng bộ seek (khởi động song song P1 bằng dữ liệu local) | tốt nhất sau P1 | FE 2-3 ngày |
...
| **P4** | Calibration: win-prob classification, opening book, telemetry fallback-rate; nghiên cứu Pikafish on-device (tùy chọn) | P1 | mở |
