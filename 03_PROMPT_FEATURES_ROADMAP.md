...
>
> **Backend foundation cho Cờ Úp online + Bot (2026-06-25, đợt 2):** engine cup được port sang TS ([cupGame.ts](cchess-backend/src/engine/cupGame.ts)) với **shuffle + hidden-assignment do SERVER giữ** — client không bao giờ biết trước danh tính ẩn (chống gian lận). `match.ts` chọn engine theo `room.variant`, mỗi nước trả **reveal** (danh tính quân vừa lộ + quân bị ăn); reconnect gửi `cup` snapshot công khai (mặt phủ + quân đã lộ + danh sách ô úp). Matchmaking **không ghép chéo** cup ↔ standard, bucket theo **`eloCup`**; persistence ghi `eloCup` pool riêng + record `variant:'cup'`. Mấu chốt: luật sinh nước cờ úp chỉ phụ thuộc mặt phủ + quân đã lộ (KHÔNG phụ thuộc danh tính ẩn) → client vẫn validate local được nếu có engine cup riêng. **Bot Cờ Úp** ([cup_bot_engine.dart](cchess/lib/core/chess_engine/ai/cup_bot_engine.dart), `GameMode.cupVsBot`) chạy cup-minimax không gian lận: chỉ thấy mặt phủ + quân đã lộ, định giá quân úp theo kỳ vọng (~320cp).
>
> **Client online Cờ Úp DONE (2026-06-25, đợt 3) — A3 end-to-end:** engine cup phía client [cup_client_game.dart](cchess/lib/core/chess_engine/cup_client_game.dart) (`CupClientGame implements ChessGameSession`) là view **cover-only** đúng như người chơi — không bao giờ biết danh tính ẩn; áp `reveal` từ `move-ack`/`opponent-move` để lật quân, dựng bàn từ `cup` snapshot khi reconnect/spectate. Luật sinh nước/chiếu tách ra [cup_rules.dart](cchess/lib/core/chess_engine/cup_rules.dart) dùng chung với engine local. Controller `game` → `ChessGameSession?`, `find-match`/`create-room` mang `variant`, render mặt úp (`hiddenPositions`); backend thêm `cup` snapshot vào `spectate-started`. Vào từ Đối Đầu → "Cờ Úp Online" (`?variant=cup`). Test `cup_client_game_test.dart` (8). Còn lại của A3 chỉ là **test tay đa thiết bị**.
