...
> **Backend `npm test` tổng cộng 32/32 xanh** (`rooms.test.ts` 4 + `match.test.ts` 8 + `server.test.ts` 7 + `server.disconnect.test.ts` 2 + `server.waitingroom.test.ts` 2 + `server.liveness.test.ts` 3 + engine-service 6).
