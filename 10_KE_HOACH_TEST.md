...
>
> **Trạng thái test tự động 2026-06-15:** Backend `cd cchess-backend && npm test` → **31/31 xanh** (9 file). Flutter `cd cchess && flutter test` → **162/162 xanh** (19 file). Chi tiết ở Nhóm T (§8).
...
> **Backend `npm test` tổng cộng 31/31 xanh** (`rooms.test.ts` 3 + `match.test.ts` 8 + `server.test.ts` 7 + `server.disconnect.test.ts` 2 + `server.waitingroom.test.ts` 2 + `server.liveness.test.ts` 3 + engine-service 6).
> **Flutter `flutter test` tổng cộng 162/162 xanh** (19 file — `online_match_controller_test.dart` 20 test).
