...
>
> **Trạng thái test tự động 2026-06-19:** Backend `cd cchess-backend && npm test` → **55/55 xanh** (9 file). Flutter `cd cchess && flutter test` → **205/205 xanh** (20 file). Chi tiết ở Nhóm T (§8).
...
> **Backend `npm test` tổng cộng 55/55 xanh** (`rooms.test.ts` 4 + `match.test.ts` 8 + `matchmaking.test.ts` 7 + `elo.test.ts` 6 + `server.test.ts` 14 + `server.disconnect.test.ts` 5 + `server.waitingroom.test.ts` 2 + `server.liveness.test.ts` 3 + engine-service 6).
