...
>
> **Trạng thái test tự động 2026-06-14:** Backend `cd cchess-backend && npm test` → **30/30 xanh** (9 file). Flutter `cd cchess && flutter test` → **160/160 xanh** (19 file). Chi tiết ở Nhóm T (§8).
...
> **Bug D1 (test tay 2026-06-13) → ĐÃ SỬA, chờ retest + REDEPLOY RENDER:** Triệu chứng: B mất **~3 phút** mới thấy banner mất kết nối, và A bật lại wifi **không chơi tiếp được** (không nhận nước đi). Nguyên nhân: (1) server chỉ phát hiện rớt qua TCP timeout vì ping/pong **WS control-frame bị proxy Render che**; (2) client **không có heartbeat riêng** và **không tự reconnect giữa ván** (chỉ reconnect khi mở lại app từ lobby). Sửa: **heartbeat tầng ứng dụng 2 đầu** (client gửi `{ping}` mỗi 5s, server `{pong}` + sweep `lastSeenAt`>15s → terminate ngay) ⇒ phát hiện ~15s; client thêm **watchdog 15s + auto-reconnect giữa ván** (retry `connect→auth→reconnect-room`) + **`connectivity_plus`** nối lại tức thì khi mạng về; guard tránh push trùng màn chơi. ⚠️ **Server chạy từ `dist/` → bắt buộc `npm run build` + REDEPLOY Render** mới có hiệu lực (heartbeat WS cũ không đủ). Test tự động: **T15** (backend liveness 2 + controller reconnect 4).

- [ ] **D1 — Phát hiện rớt nhanh + banner.** Đối thủ tắt mạng → trong **~15s** (trước đây ~3 phút) mình thấy banner vàng **"Đối thủ mất kết nối — còn 60s…"** đếm ngược. *(sửa 06-13 — chờ retest sau redeploy Render).*
- [ ] **D2 — Reconnect kịp (mở lại app HOẶC bật lại wifi giữa ván).** Trong 60s: bật lại mạng khi app vẫn mở → A tự thấy "đang kết nối lại…" rồi ván tiếp tục đúng thế cờ/đồng hồ; HOẶC mở lại app → auto-reconnect từ lobby. Banner phía B biến mất (`peer-reconnected`). *(mở rộng 06-13: thêm auto-reconnect giữa ván, không cần đóng/mở app).*
...
- [x] **T15 — D1 reconnect rớt-mạng-giữa-ván** (mới 2026-06-14 sau bug test tay Nhóm D): backend [`server.liveness.test.ts`](cchess-backend/src/server.liveness.test.ts) — client im lặng quá `LIVENESS_TIMEOUT_MS` → server terminate, peer nhận `peer-disconnected` **<2s**; client gửi `ping` → nhận `pong` và KHÔNG bị terminate *(2 test)*; Flutter `online_match_controller_test.dart` nhóm "D1 — mid-game auto-reconnect": mất kết nối khi đang chơi → `reconnecting`, `authed` tự gửi `reconnect-room`, `reconnected` → về `playing`; server từ chối (hết grace) → dừng retry + clear store; không còn fresh state → bỏ cuộc; mất kết nối ngoài ván → KHÔNG reconnect *(4 test)*. *(6 test, pass)*

> **Backend `npm test` tổng cộng 30/30 xanh** (`rooms.test.ts` 3 + `match.test.ts` 8 + `server.test.ts` 7 + `server.disconnect.test.ts` 2 + `server.waitingroom.test.ts` 2 + `server.liveness.test.ts` 2 + engine-service 6).
> **Flutter `flutter test` tổng cộng 160/160 xanh** (19 file — `online_match_controller_test.dart` 18 test).
