...
> Tài liệu sống — tạo ngày **2026-06-07**, cập nhật **2026-06-20** (chốt đợt tự động hóa T16–T25 + engine smoke quota).
...
> **Kế hoạch tự động hóa 2026-06-20:** Sau khi bổ sung integration test M1/M3 và đối chiếu coverage T16–T25, bảng chính còn **3 case chưa PASS** (§9): D4 OS lifecycle thật, M5 Firebase thật, H4 chất lượng chủ quan. Các case C/D/M/G còn lại đã có test tự động đủ để đóng.
...
- [ ] **M5 — ELO 2 chiều.** Lõi đã AUTO PASS bằng `elo.test.ts`, `persistence.test.ts` fake-store và `server.test.ts` fake persist (`game-ended.elo` đúng shape). **Chưa đóng case này** vì phần ghi `eloChess` + counters lên **Firestore thật** cần smoke/ranked-write với Firebase thật trước release.
...
- [ ] **H4 — Đánh giá lại tốc độ/chất lượng gợi ý sau tuning.** Kỳ vọng sau fix: gợi ý offline trả về trong ~0.5–2.5s (không còn +1.2s delay); nước gợi ý không còn ngẫu nhiên kém; tàn cuộc gợi ý sâu hơn. Với Pikafish server-side, dùng `engine-smoke` + vài thế cố định để kiểm hợp lệ/tốc độ, nhưng "nước có hay" vẫn cần người chơi hoặc engine mạnh đối chiếu.
...
| D — Reconnect | 1 | D2/D3/D5 qua liveness/drop/reconnect/double-disconnect integration + controller/widget | **D4** Home/Resume/kill app/đổi Wi-Fi/4G thật |
| M — Matchmaking/ELO | 1 | M1–M4 qua matchmaking unit + WebSocket integration + controller | **M5** Firestore thật/ranked-write với 2 Firebase user thật trước release |
...
| H — Gợi ý | 1 | H1–H3 + logic hint/router; engine smoke kiểm hợp lệ/tốc độ service | **H4** chất lượng nước gợi ý cần người chơi/engine mạnh đối chiếu |

**Kết luận khuyến nghị:** giữ 3 mục chưa PASS là đúng ranh giới: **D4 lifecycle/OS thật**, **M5 với hạ tầng Firebase thật**, **H4 chất lượng chủ quan**. Các mục còn lại đã đủ điều kiện đóng bằng auto-test và nên tiếp tục chạy trong CI.
...
| D — Reconnect | 5 | 4 | 0 | 1 (D4 — OS lifecycle thật) |
| M — Matchmaking/ELO | 5 | 4 | 0 | 1 (M5 — Firebase thật/ranked-write) |
| G — Lifecycle | 6 | 6 ✅ | 0 | 0 |
| H — Gợi ý in-game | 4 | 3 | 0 | 1 (H4 — đánh giá sau tuning) |
| T — Test tự động | 25 | 25 | 0 | 0 |
| **Tổng** | **80** | **77** | **0** | **3** |

> Cập nhật bảng này sau mỗi đợt test. Sau đợt auto-test 2026-06-20, chỉ còn **D4** (OS lifecycle/mạng thật), **M5** (Firestore/ranked-write thật với 2 Firebase user), **H4** (chất lượng gợi ý chủ quan). Không cố ép 3 mục này thành unit test giả.
...
Cập nhật 2026-06-19/20 (T16–T25): tự động hóa backend M/G/ELO, Flutter controller/widget, persistence adapter/idempotency, smoke deploy/engine, **G1 checkmate** bằng fixture FEN + integration WS, quota smoke gate cho `cchess-engine`, và thêm WebSocket integration cho M1/M3. Backend `npm test` 32→**71 xanh**, Flutter online suite **101 xanh** (`flutter test test/online`; full suite gần nhất 226 xanh), các lệnh analyze/lint sạch; engine product smoke 8/8 PASS trên Render. Lần cập nhật kế tiếp: chạy `post-deploy-smoke` ranked-write sau redeploy/staging nếu cần, hoặc phiên test tay còn giữ D4 OS-thật / M5 Firebase thật / H4 chất lượng gợi ý.*
