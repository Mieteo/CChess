...
>
> **Trạng thái test tự động 2026-06-19:** Backend `cd cchess-backend && npm test` → **68/68 xanh** (12 file). Flutter `cd cchess && flutter test` → **226/226 xanh** (21 file). Chi tiết ở Nhóm T (§8).
...
> **Backend `npm test` tổng cộng 68/68 xanh** (`rooms.test.ts` 4 + `match.test.ts` 9 + `matchmaking.test.ts` 7 + `elo.test.ts` 6 + `persistence.test.ts` 10 + `server.test.ts` 16 + `server.disconnect.test.ts` 5 + `server.waitingroom.test.ts` 2 + `server.liveness.test.ts` 3 + engine-service 6).
...
Cập nhật 2026-06-19 (T16–T23): tự động hóa backend M/G/ELO, Flutter controller/widget, persistence adapter/idempotency, smoke deploy/engine, và **G1 checkmate** bằng fixture FEN + integration WS. Backend `npm test` 32→**68 xanh**, Flutter `flutter test` 163→**226 xanh**, các lệnh analyze/lint sạch. Lần cập nhật kế tiếp: chạy smoke staging/prod thật, hoặc phiên test tay còn giữ D4 OS-thật / M5 Firebase thật / H4 chất lượng gợi ý + một vòng nhìn-mắt cuối cho C2/D/G4 trên thiết bị.*
