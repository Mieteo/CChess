...
> Tài liệu sống — tạo ngày **2026-06-07**, cập nhật **2026-06-20** (chốt đợt tự động hóa T16–T24 + engine smoke quota).
...
>
> **Trạng thái test tự động 2026-06-20:** Backend `cd cchess-backend && npm test` → **69/69 xanh** (12 file). Flutter `cd cchess && flutter test` → **226/226 xanh** (21 file). Chi tiết ở Nhóm T (§8).
...
> **Kế hoạch tự động hóa 2026-06-20:** Tổng còn lại vẫn là **23 case chưa PASS test tay** (§9), nhưng phần lõi C/D/M/G/H đã được phủ bằng backend lab/integration, Flutter controller/widget test, persistence/ELO test và smoke script staging/prod. Các case còn giữ lại chủ yếu là OS-thật/Firebase-thật/chất lượng chủ quan/nhìn-mắt cuối. Xem §8b.
...
> **Trạng thái 2026-06-20:** T1–T24 đều xanh. Backend `cd cchess-backend && npm test` → **69/69** (12 file). Flutter `cd cchess && flutter test` → **226/226** (21 file). `backend-ci` còn chạy thêm lab/load/fuzz ngoài `npm test`.
...
> **Backend `npm test` tổng cộng 69/69 xanh** (`rooms.test.ts` 4 + `match.test.ts` 9 + `matchmaking.test.ts` 7 + `elo.test.ts` 6 + `persistence.test.ts` 10 + `server.test.ts` 16 + `server.disconnect.test.ts` 5 + `server.waitingroom.test.ts` 2 + `server.liveness.test.ts` 3 + engine-service 7).
...
Cập nhật 2026-06-11 → 2026-06-18: hardening double-disconnect (D5), nút Gợi ý/chip chat, test tay Nhóm R/S, fix R9, tuning H1–H3, và thêm §8b để chuyển dần 23 case còn lại sang backend lab, Flutter controller/widget test, persistence/ELO test, smoke/staging script.
Cập nhật 2026-06-19/20 (T16–T24): tự động hóa backend M/G/ELO, Flutter controller/widget, persistence adapter/idempotency, smoke deploy/engine, **G1 checkmate** bằng fixture FEN + integration WS, và quota smoke gate cho `cchess-engine`. Backend `npm test` 32→**69 xanh**, Flutter `flutter test` 163→**226 xanh**, các lệnh analyze/lint sạch; engine product smoke 8/8 PASS trên Render. Lần cập nhật kế tiếp: chạy `post-deploy-smoke` ranked-write sau redeploy/staging nếu cần, hoặc phiên test tay còn giữ D4 OS-thật / M5 Firebase thật / H4 chất lượng gợi ý + một vòng nhìn-mắt cuối cho C2/D/G4 trên thiết bị.*
