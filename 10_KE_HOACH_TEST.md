...
>
> **Trạng thái test tự động 2026-06-25:** Backend `cd cchess-backend && npm test` → **117/117 xanh** (19 file). Flutter `cd cchess && flutter test` → **305/305 xanh** (31 file). Chi tiết Nhóm T (§8) là snapshot online 2026-06-20; các đợt sau bổ sung shop (S16) + Cờ Úp luật Sĩ/Tượng ngửa (S13).
...
### 8b.7. Phase F — Auto-test cho các commit sau S15/B1+B4/S13/S14

**Mục tiêu:** khóa lại các thay đổi lớn gần đây bằng test tự động theo mức rủi ro. Ưu tiên cao nhất là luật cờ/engine; các màn dữ liệu và nội dung học triển khai sau.

- [x] **P-F1 — S13 Cờ Úp: luật đi quân/reveal/undo + Sĩ/Tượng ngửa tự do.** Ưu tiên P0. Auto-test trong `xiangqi_cup_game_test.dart` phủ: quân úp đi nước đầu theo quân vỏ, sau khi lật đi theo quân thật, bắt quân úp trả về quân thật bị bắt, undo khôi phục hidden map, shuffle giữ đúng multiset quân mỗi bên, legality không để reveal tạo thế tự chiếu/lộ mặt tướng; **và (2026-06-25) nhóm "revealed Sĩ/Tượng roam freely": Sĩ/Tượng ĐÃ MỞ thoát giới hạn cung/sông (Sĩ chéo 1, Tượng chéo 2 + cản mắt, đi khắp bàn, chiếu Tướng được), còn quân CÒN ÚP trên ô Sĩ/Tượng vẫn bị giam theo mặt phủ.**
- [ ] **P-F2 — S15 engine/AI Coach/quota.** Ưu tiên P1. Bổ sung fixture contract cho `EngineRouter`, quota/VIP, fallback remote→local, FEN/UCI và `CoachAnalyzer` trên vài kỳ phổ cố định.
- [ ] **P-F3 — B1+B4 học tập/tàn cục.** Ưu tiên P2. Bổ sung test catalog/content: lesson id/order unique, link bài học→puzzle không chết, puzzle seed có solution legal, và bài tập không bắt đầu từ trạng thái vô nghĩa.
- [ ] **P-F4 — S14 Cộng Đồng.** Ưu tiên P2. Bổ sung test repository/routing: friends fallback, leaderboard sort/scope, friend request payload, route con `/community/*`, và Firebase rules/functions bằng emulator khi bắt đầu dùng dữ liệu cloud thật.
