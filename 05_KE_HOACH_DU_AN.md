...
> Tài liệu sống — cập nhật ngày **2026-08-07** (đối chiếu lại `git log` + chạy lại test): HEAD hiện tại `3258140` trên `main`, đồng bộ `origin/main`, working tree sạch. Mốc gần nhất **2026-07-28**: polish thuần UI — quân cờ vẽ lại bằng `_TiltedCylinderPiecePainter` (hiệu ứng gỗ sơn mài 3D thay Container gradient phẳng), `pieceDiameter` 0.90, kèm bộ widget test + golden preview mới cho `ChessPieceWidget` (nằm ngoài khung sprint chính thức, thuộc mảng UI Sprint 4). Mốc trước **2026-07-23 (tối)** — **đợt review toàn luồng người dùng trên emulator, đóng hết P1 (5 commit) + P2 (6 commit)**: PopScope thoát ván, 3 nút app-bar (avatar/chuông/gear) hết chết, lobby auto-connect, lỗi mua hiện trong sheet, banner điểm danh/thử thách dùng data thật; rồi leave-ván-bot = resign thật, khoá undo trong ván rated, overlay xếp dọc, đếm nước tàn cục, Balo/RewardChips tra tên đẹp từ catalog, thống nhất đơn vị "xu", route Giải Đấu/Kỳ Phổ, `sendMail` nhận `dedupeKey` chống trùng broadcast, và fix root cause Duplicate GlobalKey (bỏ `AnimatedSwitcher` giữ subtree tab cũ trong AppShell). **Còn lại duy nhất không sửa được từ code:** Google Sign-In `DEVELOPER_ERROR` trên emulator — cần thêm SHA-1 debug keystore vào Firebase console (việc tay, không phải code). Xác nhận lại test **2026-08-07**: Flutter `flutter test` **464 test, 463 pass / 1 fail** (fail ở `bot_engine_test.dart` — test nhạy thời gian "still honors the minimum think time", không phải regression tính năng); backend `npm test` **232 test, 231 pass / 1 fail** (fail ở `server.test.ts` M1/M4 matchmaking timeout chờ message — cũng dạng flaky-timing). Mốc trước đó cùng ngày 2026-07-23 (chiều): **Sprint 16 hoàn tất phần code** — thêm 4 mảnh economy còn thiếu **D4 Hộp Thư / D5 Sự Kiện / D6 Phúc Lợi (điểm danh) / D7 Đúc Bàn Cờ** (backend module `economy/` + 4 màn Flutter + rules + seed import; backend `npm test` **206/206**, Flutter **434 pass**, analyze/tsc sạch). Cùng ngày: Android QA Pikafish Offline **PASS** (doc 15). Mốc trước **2026-07-01** (xem mục thay đổi ở cuối: Sprint 14 Community **C3 Kỳ Xã + C4 Giải Đấu + C6 Tin tức/Tàn Cục hàng ngày** đưa từ read-only stub lên backend thật — module `cchess-backend/src/{clubs,community_feed,tournaments}/` + bracket đấu loại trực tiếp reuse phòng casual có sẵn — backend `npm test` **177/177**, Flutter `flutter test` **375/375**). Mốc trước đó cùng ngày: máy dev thứ 2 cài đầy đủ toolchain Node/JDK/Firebase CLI, xác nhận lại xanh 100% — Flutter `flutter test` **354/354**, backend `npm test` **147/147** — và deploy `firestore:rules,indexes`+TTL lên `cchess-dev`, đóng nốt việc tồn đọng Sprint 15. Mốc trước **2026-06-26**: hệ Bot ELO liên tục Phase 0–6, ElephantEye native, Zobrist/TT, quota Firestore TTL, ví đồng bộ. Mốc trước nữa **2026-06-20**: chốt đợt T16–T24. Backend `npm test` **86/86** (14 file), Flutter `flutter test` **233/233** (22 file); `backend-ci` đã chạy `lint` + `lab:check` + `npm test` + lab/load/fuzz, `flutter-ci` chạy analyze/test, thêm gate thủ công `post-deploy-smoke` và `engine-smoke`. Engine service `https://cchess-engine.onrender.com` đã smoke thật **8/8 PASS** gồm quota `429 quota-exceeded`. Việc còn lại chuyển sang hardening sản phẩm: quota/VIP bền vững, license NNUE, test tay cuối D4/M5/H4 và AI Coach B3.
...
| 4 | ChessBoard UI (CustomPainter) | ✅ | `widgets/chess/`, `presentation/game/`. **Polish 2026-07-28**: quân cờ vẽ lại 3D (`_TiltedCylinderPiecePainter`, sơn mài gỗ) thay Container gradient phẳng |
...
| 12 | Online Matchmaking + Spectate (A1, A5, A6) | 🟡 | **A1 Ranked done**; **A5 chat cơ bản + chip nhanh done**; **A6 Spectate/share link/QR done**; **Rematch done**; R/S đã đóng test tay. Đợt 2026-06-19/20 đã tự động hóa lõi C/D/M/G/ELO + widget online + smoke deploy. **Đợt review luồng 2026-07-23**: lobby auto-connect (URL editor chỉ debug), PopScope thoát ván đi qua xác nhận, fix root cause Duplicate GlobalKey (AnimatedSwitcher giữ subtree tab cũ). Còn lại: vòng nhìn-mắt cuối trên thiết bị thật cho D4 OS lifecycle, M5 Firebase thật, H4 chất lượng gợi ý, C2/D/G4 visual + nâng Render Starter khi có user thật |
...
| 14 | Community (Bạn bè, Leaderboard, CLB) | ✅ | **C1–C4, C6 đã nối backend thật (2026-07-01)**: friend request/accept/decline/remove ghi Firestore (`friendships`, batch 2 chiều); Cloud Function `syncCommunityProfile` mirror `users`→`public_profiles`→`leaderboards`; **C3 Kỳ Xã** tạo/tham gia/rời (giới hạn 3 CLB/người); **C4 Giải Đấu** đăng ký + bracket đấu loại trực tiếp thật (reuse phòng casual, tự tính bye/nâng hạng); **C6** feed admin-biên tập + trỏ vào tàn cục hàng ngày thật (B4). **Còn:** C5 Livestream (Sprint 18), nội dung tin tức thật, test tay thiết bị |
...
| 16 | Khám Phá (Shop, Inventory, Mail, Event) | ✅ | **Shop + Inventory + Explore UI + repo + backend routes/store + shop.test.ts DONE** (route `/shop` `/inventory`, Explore là hub); **ví đồng bộ từ backend** (2026-06-26); **D4 Hộp Thư + D5 Sự Kiện + D6 Phúc Lợi (điểm danh chuỗi 7 ngày VN-midnight, quà tân thủ, quà quay lại ≥7 ngày) + D7 Đúc Bàn Cờ DONE 2026-07-23**: module backend `cchess-backend/src/economy/` (claim-once transaction + `wallet_tx` audit, admin CRUD + `economy:import` seed), Flutter `presentation/economy/` 4 màn + Explore hub tiles + badge thư chưa đọc, rules Firestore mail/event_claims/welfare/events/craft_recipes. **Fix routing:** puzzle API từng chiếm toàn bộ `/admin/*` làm 404 admin shop/feed qua server thật. **Đợt review 2026-07-23**: `sendMail` nhận `dedupeKey` chống trùng broadcast, Balo/RewardChips tra tên đẹp catalog thay raw id, thống nhất đơn vị "xu". Còn: nội dung event/recipe thật (seed đã có), Mail broadcast tự động từ giải đấu |
...
**Đã làm — A2 Cờ Casual + Bot Cờ Úp expectiminimax (2026-06-25, đợt 4):**
- **A2 Cờ Casual (Cờ giao hữu, không tính ELO)**: vào từ Đối Đầu → **"Mời Bạn Đấu"** (`?casual=1` → [online_lobby_screen.dart](cchess/lib/presentation/online/online_lobby_screen.dart) `initialCasual`). Tạo **phòng riêng casual** rồi mời bạn qua **link/QR/mã phòng 6 ký tự** (`ShareRoomSheet`/`RoomShare`); bạn vào bằng deep-link `?join=ID` hoặc nhập mã. Backend (`server.ts`/`rooms.ts`) đã có `RoomMode='casual'` — phòng casual **bỏ qua persist + không phát ELO** (`finishGame` guard `room.mode==='casual'`), `game-ended` mang `mode:'casual', elo:null`. Client đánh dấu casual khắp nơi (app bar "Casual", bỏ refresh ELO sau ván, **dialog kết quả hiện rõ "Cờ giao hữu — không tính ELO"**, hint ở phòng chờ). Test: backend `server.test.ts` **S13** (casual skip persist + elo null), Flutter `online_match_controller_test.dart` nhóm **A2** (roomMode→isCasual qua room-created/game-start/game-ended) + `online_game_widgets_test.dart` (dialog casual note).
- **Bot Cờ Úp v2 — expectiminimax đầy đủ** ([cup_bot_engine.dart](cchess/lib/core/chess_engine/ai/cup_bot_engine.dart)): thay minimax v1 ("lật-thành-mặt-phủ" + định giá phẳng 320cp). Nay **mỗi nước lật là một chance node** — phân nhánh theo các loại quân còn trong "túi ẩn" của bên đi, trọng số theo số lượng còn lại, lấy kỳ vọng (expectiminimax MAX/MIN + chance). Quân **còn úp** định giá bằng **trung bình túi ẩn hiện tại** (`M0` 15 quân trừ quân đã lộ trên bàn) nên giá trị **dịch chuyển khi quân lộ dần** (vd 2 Xe đã lộ → phần còn lại rẻ hơn). **Không gian lận** (chỉ dùng mặt phủ + quân đã lộ + túi suy từ bàn nhìn thấy). Iterative deepening + **ngân sách thời gian** (300–2000ms theo cấp độ) nên không treo dù thế khai cuộc đầy quân úp; giữ nguyên các núm randomChance/suboptimalChance/minThinkTime. Test: `cup_bot_engine_test.dart` (+ "prefers a known high-value capture over a face-down one (no peeking)", + "searches a full face-down opening at depth without hanging").

**Còn lại:** chỉ test tay đa thiết bị cho ván casual/cup online thật.

### ✅ Sprint 14 — Community (Module C) — C1–C4, C6 đã nối backend thật
...
- ✅ **(2026-07-01) C3 Câu lạc bộ Kỳ Xã** — module backend mới `cchess-backend/src/clubs/` (types/store/routes, theo mẫu `shop/`): `GET /clubs`, `GET /clubs/:id`, `GET /clubs/:id/members`, `GET /clubs/mine`, `POST /clubs` (tạo, người tạo thành `owner`), `POST /clubs/:id/join`, `POST /clubs/:id/leave` — transaction giữ `memberCount` nhất quán + giới hạn **tối đa 3 Kỳ Xã/người**; owner không rời được khi CLB còn thành viên khác (chưa hỗ trợ chuyển quyền sở hữu — v2). Flutter: `club_repository.dart` + `clubs_api_source.dart` (cache Hive `cchess_clubs`), `clubs_screen.dart` tạo/tham gia/rời thật (không còn SnackBar giả), màn mới `club_detail_screen.dart` (danh sách thành viên + bảng điểm tĩnh). Rules `clubs/{id}/members/{uid}` (đọc công khai, ghi qua Admin SDK).
- ✅ **(2026-07-01) C4 Giải đấu định kỳ** — module backend mới `cchess-backend/src/tournaments/` gồm thuật toán bracket **thuần túy, tách riêng** (`bracket.ts`, test độc lập không cần Firestore): đấu loại trực tiếp, tự tính bye khi số người không phải lũy thừa 2 (mỗi cặp tối đa 1 bye, xác nhận qua test `n=5 → 3 bye`), cascade nâng hạng người thắng vào vòng sau qua `nextMatchId`/`nextMatchSlot` tính sẵn lúc tạo bracket. Trận đấu tái dùng **luồng phòng riêng casual có sẵn** (A2) thay vì xây matchmaking mới: `Room` (`rooms.ts`) thêm field tùy chọn `tournamentTag`, `create-room` gắn tag → tự chuyển `mode:'casual'` (không tính ELO thang) + gọi `attachRoomToMatch` để đối thủ thứ 2 biết roomId; `finishGame` gọi `recordMatchResult` nâng bracket khi ván kết thúc — hòa thì reset trận về `ready` để đấu lại (đấu loại trực tiếp không có hòa). Endpoint: `GET /tournaments(,/:id,/:id/participants,/:id/matches)` công khai; `POST /tournaments`, `POST /tournaments/:id/start` chỉ admin-key (v1 **chỉ giải hệ thống tổ chức**, "giải do người dùng tự tổ chức" ngoài phạm vi); `POST /tournaments/:id/{register,unregister}` cần đăng nhập, kiểm tra hạn đăng ký/khoảng ELO/sức chứa/trùng lặp. Flutter: `tournament_repository.dart` + `tournaments_api_source.dart`, `tournaments_screen.dart` đăng ký/hủy thật, màn mới `tournament_detail_screen.dart` (bracket dạng danh sách theo vòng + nút "Vào trận" dẫn vào `online_lobby_screen.dart` đã mở rộng tham số `tournamentId`/`matchId` để tự tạo/vào phòng gắn tag). Rules `tournaments/{id}/{participants,matches}/*`.
- ✅ **(2026-07-01) C6 Tin tức + Tàn Cục Thách Đấu hàng ngày** — module backend mới `cchess-backend/src/community_feed/` (nội dung do admin biên tập, giống `shop_items`/`puzzles`): `GET /community/feed` công khai, `POST/PUT/DELETE /admin/community/feed` admin-key. Thẻ "Tàn Cục Thách Đấu" dùng field `route:'daily_puzzle'` để trỏ sang **tàn cục hàng ngày thật đã có sẵn** (`GET /puzzles/daily`, B4) thay vì trùng lặp logic — không xây lại daily-puzzle. `community_repository.dart.loadFeed()` chuyển thành async: backend → fallback 3 thẻ cứng khi lỗi mạng.
- ⬜ **C5 Livestream** — vẫn chưa làm (Sprint 18 theo kế hoạch gốc).
- ⬜ Nội dung tin tức thật (2-3 bài xác thực đăng qua `POST /admin/community/feed` sau khi deploy — việc biên tập nội dung, không phải code) + xác nhận NNUE/vận hành thật khi có giải chạy live trên thiết bị.

**Test:** backend `npm test` **177/177** (thêm `clubs.test.ts` 7, `community_feed.test.ts` 5, `tournaments/tournament.test.ts` 16 gồm test thuật toán bracket thuần túy, `server.test.ts` +2 test tích hợp WS cho `tournamentTag`), Flutter `flutter test` **375/375** (thêm test cho `club_repository`/`clubs_api_source`/`community_feed_api_source`/`tournaments_api_source`/`tournament_repository`). `flutter analyze` + `tsc --noEmit` sạch. **Chưa test tay trên thiết bị/emulator thật** — môi trường chạy phiên này không có mạng ra Firebase để load app lên trình duyệt xác nhận trực quan; cần người dùng tự xác nhận UI khi có điều kiện.
...
- ✅ **(2026-07-01)** `firestore:rules,indexes` + TTL policy cho `engine_usage` đã **deploy lên `cchess-dev`** (`firebase deploy --only firestore:rules,firestore:indexes` — rules compiled/released, indexes + TTL policy deployed thành công). **Còn lại:** deploy tương tự lên `cchess-prod` khi cần, xác nhận **NNUE license thương mại**, đối chiếu FEN/UCI thêm nhiều thế + **H4 chất lượng gợi ý/calibrate** với Pikafish thật, upgrade engine lên Render **Standard** trước traffic thật.

### ✅ Sprint 16 — Khám Phá (Module D)
**Đã làm:** Shop (D1) + Inventory (D2) + màn **Explore** (hub) — UI `presentation/shop/` (`shop_screen`, `inventory_screen`, `explore_screen`, `shop_visuals`, `shop_controller`); models `shop_item`/`inventory_item`/`wallet`; `shop_repository` + `shop_api_source`; route `/shop` `/inventory` trong `app_router`. Backend `src/shop/` (routes/store/types) + `scripts/import_shop.ts` + `shop.seed.json` + `shop.test.ts`; `firestore.rules` cho inventory/ví. **DONE 2026-07-23**: D4 Hộp Thư + D5 Sự Kiện + D6 Phúc Lợi (điểm danh chuỗi 7 ngày) + D7 Đúc Bàn Cờ — module backend `economy/` + 4 màn Flutter + Explore hub tiles + badge thư chưa đọc; `sendMail` nhận `dedupeKey` chống trùng broadcast (fix 2026-07-23 tối); Balo/RewardChips tra tên đẹp catalog.
**Chưa làm:** D3 Khung Avatar/danh hiệu gắn thật (hiện theo shop kinds có sẵn); nội dung event/recipe thật ngoài seed mẫu; Mail broadcast tự động từ hệ thống giải đấu; quyết định **tab thứ 6 hay gộp** (hiện 5 tab).
...
| A2 | Cờ Casual + mời bạn | 🟢 | 13 — **DONE 2026-06-25**: phòng riêng **không tính ELO** (server bỏ persist + `elo:null`), mời bạn qua link/QR/mã phòng, vào từ "Mời Bạn Đấu". Còn: test tay đa thiết bị |
...
| C3 | Câu lạc bộ Kỳ Xã | ✅ | 14 — **DONE 2026-07-01**: tạo/tham gia/rời CLB thật (`cchess-backend/src/clubs/`), giới hạn 3 CLB/người, màn chi tiết + danh sách thành viên |
| C4 | Giải đấu | ✅ | 14 — **DONE 2026-07-01**: đăng ký + bracket đấu loại trực tiếp thật (`cchess-backend/src/tournaments/`), trận đấu qua phòng casual gắn tag, chỉ giải hệ thống tổ chức (v1) |
| C5 | Livestream | ⬜ | 18 |
| C6 | Tin tức + Tàn cục hàng ngày | ✅ | 14 — **DONE 2026-07-01**: feed admin-biên tập (`cchess-backend/src/community_feed/`) trỏ vào tàn cục hàng ngày thật (B4); còn thiếu nội dung tin tức thật |
| C7 | Diễn đàn | ⬜ | 18 |
...
| D3 | Khung Avatar / danh hiệu | 🟡 | 16 — vẫn theo shop kinds có sẵn, chưa có khung/danh hiệu riêng |
| D4–D7 | Mail, Event, Welfare, Crafting | ✅ | 16 — **DONE 2026-07-23**: module backend `economy/` (claim-once + `wallet_tx` audit, admin CRUD, `dedupeKey` chống trùng broadcast) + 4 màn Flutter + Explore hub tiles + badge thư chưa đọc |
| E1 | Hồ sơ + cấp bậc | ✅ | 7 |
...
## 6. Số liệu tổng (cập nhật 2026-08-07)

- **Git HEAD:** `3258140` trên `main`, đồng bộ `origin/main`, working tree sạch.
- **Tổng file Dart `lib/`:** ~110+ file (thêm từ đợt trước: `core/chess_engine/` engine lai + ELO ladder: move_engine / engine_router / local_minimax_engine / local_elephanteye_engine + `ffi/` / remote_pikafish_engine / `ai/engine_config` / `matchmaking/{bot_matchmaker,elo_scoring}` / `presentation/calibration/` / `presentation/economy/` (Mail/Event/Welfare/Crafting)).
- **Backend TypeScript:** realtime server + lab + engine-service + `economy/` + `clubs/` + `community_feed/` + `tournaments/` (server, UCI wrapper, pool, analysis, cache, quota in-memory **+ FirestoreQuotaStore bền vững**, FEN) đã có CI riêng.
- **Test tự động (xác minh lại 2026-08-07 bằng cách chạy trực tiếp `npm test` / `flutter test`):**
  - Backend `npm test` **232 test — 231 pass / 1 fail** (fail: `server.test.ts` M1/M4 matchmaking, "timeout waiting for message" — dạng flaky-timing của integration test, không phải regression tính năng; chạy lại để xác nhận trước khi coi là bug thật).
  - Flutter `flutter test` **464 test — 463 pass / 1 fail** (fail: `bot_engine_test.dart` "best-effort (hint) mode ... honors the minimum think time" — cũng nhạy timing, cùng lý do trên).
  - `flutter analyze` / `tsc --noEmit` sạch (chưa chạy lại trong phiên 2026-08-07, giữ nguyên trạng thái xanh từ lần merge gần nhất).
- **Test tay:** R **ĐÓNG 12/12**; S **ĐÓNG 15/15**; C8 + H1–H3 PASS; **đợt review toàn luồng 2026-07-23 đóng P1 (5) + P2 (6) bug UX trên emulator**. Còn lại chủ yếu là vòng thật/visual: D4 OS lifecycle, M5 Firebase thật, H4 chất lượng gợi ý, C2/D/G4 nhìn-mắt cho ván cup/casual online, và **SHA-1 debug keystore cho Google Sign-In** (việc tay Firebase console, không phải code).
- **Sprint hoàn thành (1 chiều):** 10/18 (1–7 + 8a + 8b + 8c).
- **Sprint code xong, sync một phần:** 3/18 (S9, S10, S11).
- **Sprint MVP/code done, chỉ còn việc tay:** S12 (A1 Ranked production — còn test đa thiết bị cuối + Render Starter khi có user thật), S13 (A3 Cờ Úp + A2 Casual end-to-end — còn test tay đa thiết bị), S14 (Community C1–C4, C6 nối Firestore thật — còn C5 Livestream dời S18 + nội dung tin tức thật), S16 (Shop/Inventory/Mail/Event/Welfare/Crafting toàn bộ D1–D2, D4–D7 — còn D3 khung avatar + nội dung event/recipe thật).
- **Sprint đang dở (còn việc code/ops thật sự):** S15 (engine lai + AI Coach B3 + quota/VIP bền vững đã xong; còn NNUE license thương mại, Render Standard, **calibrate ELO ladder thủ công**, H4 chất lượng gợi ý).
- **Sprint chưa làm:** S17 (VIP/IAP — chờ store account), S18 (OCR, học kỳ phổ, Livestream, Diễn đàn — giai đoạn 3); nội dung B1 (khóa học vỡ lòng) vẫn chỉ là UI placeholder.
- **Tỷ lệ hoàn thành code (ước lượng theo spec MVP+G2):** ~88–90% (tăng sau khi S16 D4–D7 xong + đợt review/fix UX toàn luồng 2026-07-23 dọn sạch hầu hết nút chết/luồng giả).
- **Tỷ lệ tính năng end-to-end dùng được production:** ~65–70% (ranked online + economy + community đã chạy thật qua backend; còn thiếu hardening Render Standard/Starter, NNUE license, và một vòng test tay đa thiết bị cuối trước khi mở user thật).
...
0a. ✅ **ĐÃ FIX 2026-06-26 — ELO ván-vs-bot không còn bị splash sync ghi đè (hướng b).** Bug: sau refactor ELO ladder (Phase 3), ván bot cộng `eloDelta` vào `eloChess`/W-L-D local nhưng không đẩy được lên cloud (server-authoritative) → bị `_mergeCloudIntoLocal` ghi đè khi mở lại app. **Sửa:** tách **pool đấu bot riêng client-owned** — `eloBot` + `botGames`/`botWins`/`botLosses`/`botDraws` ([user_profile.dart](cchess/lib/data/models/user_profile.dart)); ván bot chỉ đụng pool này + đẩy lên cloud (rules cho owner ghi 5 field, vẫn khoá field ranked) + merge đọc lại từ cloud. `eloChess`/ranked W-L-D giữ server-authoritative; leaderboard chỉ dùng `eloChess`/`eloCup`. Hồ sơ + màn chọn bot hiện `eloBot`. Flutter `flutter test` **353/353**, analyze sạch. Chi tiết: [09](09_BACKEND_SERVER_HOAT_DONG.md) §8 + [13](13_KE_HOACH_ELO_BOT_LADDER.md) §6. **Deploy:** ✅ rules + indexes + functions đã lên **cchess-dev 2026-06-27** (gồm `createFirestoreUser` seed `eloBot`); **prod còn lại** (`-P prod`).
0b. ✅ **ĐÃ FIX 2026-07-23 — đợt review toàn luồng người dùng trên emulator, đóng P1 (5 commit) + P2 (6 commit).** Danh sách chi tiết ở banner đầu tài liệu; trọng tâm: mọi nút chết trong app bar đã nối route thật, dialog kết quả/ELO/casual đã minh bạch, `sendMail` chống trùng broadcast bằng `dedupeKey`, root cause Duplicate GlobalKey đã sửa tận gốc (bỏ `AnimatedSwitcher` giữ subtree tab cũ). **Việc tay duy nhất còn treo (không sửa được từ code):** thêm SHA-1 debug keystore vào Firebase console để hết lỗi `DEVELOPER_ERROR` khi Google Sign-In trên emulator.
...
3. **Vòng test tay cuối** — D4 OS lifecycle thật, M5 Firebase thật, H4 chất lượng nước gợi ý/Pikafish, nhìn-mắt C2/D/G4 trên thiết bị, và test đa thiết bị cho ván cup/casual online (S13).
4. **License/commercial check** — chốt NNUE license trước khi dùng engine trong production thương mại.
5. **Nội dung Học Cờ (B1 + B4-C)** — B1 khóa học vỡ lòng vẫn chỉ là UI placeholder, chưa có nội dung; B4 kho tàn cục đã có đủ hạ tầng (`puzzles:import`/`set-daily`) nhưng cần nhập bài dần. Đây là mảng nội dung lớn nhất còn thiếu sau khi S13/S14/S16 đã xong code.
...
6. ✅ **Friends list (C1)** — **DONE trong Sprint 14** (`friendships` Firestore 2 chiều); còn thiếu presence online/offline realtime nếu cần sau này.
...
*Cập nhật 2026-07-01 — **Máy dev thứ 2 (`D:\workspace\CChess`) cài đầy đủ toolchain backend/Firebase lần đầu**: Node.js 24 LTS + JDK 21 (Temurin) + Firebase CLI qua winget/npm, `firebase login` xong (project mặc định `cchess-dev`). Xác nhận lại **toàn bộ xanh trên máy này**: backend `npm test` **147/147**, `tsc --noEmit` sạch (gồm cả thay đổi `blunderRate` Pikafish của commit trước — trước đó chưa verify được vì máy này chưa có Node); `cchess/functions` build + lint sạch; Flutter `flutter test` **354/354**. **Deploy `firestore:rules,firestore:indexes` lên `cchess-dev` thành công** — đóng nốt việc tồn đọng của Sprint 15 (TTL policy `engine_usage.expireAt` + deny-rule client giờ đã live).*
- *Test: Flutter `flutter test` **350/350** (37 file), `flutter analyze` sạch. **Còn lại lớn:** calibrate ELO thủ công (đấu thử); Sprint 16 **D3–D7** Mail/Event/Welfare/Crafting + kinh tế mua-bán (D1/D2 Shop/Inventory/ví **đã end-to-end**); Sprint 17 VIP/IAP; content B1/B4; và vòng test tay/production-hardening Render.*

*Cập nhật 2026-07-01 (đợt 2) — **Sprint 14 Community C3/C4/C6 done**: 3 module backend mới `cchess-backend/src/{clubs,community_feed,tournaments}/` theo đúng mẫu `shop/` đã có (types/store/routes tách lớp, admin-key cho ghi biên tập, transaction Firestore cho thao tác cần nhất quán). Điểm kỹ thuật đáng chú ý — **C4 bracket đấu loại trực tiếp**: thuật toán tách hẳn khỏi Firestore vào `bracket.ts` (test thuần túy, không cần fake store/emulator) — `generateBracket()` tính bye một-bye-mỗi-cặp (không bao giờ 2 bye chung 1 cặp) và cascade nâng người thắng bye vào vòng sau ngay lúc tạo bracket; `applyMatchResult()` idempotent + xử lý hòa (reset trận, không nâng hạng — đấu loại trực tiếp không cho hòa). Trận đấu tái dùng **nguyên luồng phòng riêng casual (A2) đã có** thay vì xây matchmaking mới: `Room.tournamentTag` là field tùy chọn duy nhất thêm vào `rooms.ts`; `finishGame()` gọi `recordMatchResult` độc lập với nhánh persist ELO đã có (tournament luôn `mode:'casual'` nên không đụng ELO thang xếp hạng). Backend `npm test` **147 → 177/177** (+7 clubs, +5 community_feed, +16 tournaments gồm bracket thuần túy, +2 server.test.ts tích hợp WS thật cho `tournamentTag`), `tsc --noEmit` sạch. Flutter `flutter test` **354 → 375/375** (+11 club, +2 feed source, +5 tournaments source, +2 tournament repo), `flutter analyze` sạch. Firestore rules thêm subcollection `clubs/{id}/members`, `tournaments/{id}/{participants,matches}` (đọc công khai, ghi qua Admin SDK) — **đã `firebase deploy --only firestore:rules` lên `cchess-dev` thành công** (rules compiled + released); còn thiếu deploy tương tự lên `cchess-prod` khi cần. **Chưa làm**: test tay trên thiết bị/trình duyệt thật (phiên làm việc này không có mạng ra Firebase để xác nhận UI trực quan — thử `flutter run -d chrome` bị treo ở bước khởi tạo Firebase, không lỗi rõ ràng, nghi do sandbox chặn mạng chứ không phải lỗi code); nội dung tin tức C6 thật (đăng qua `POST /admin/community/feed` sau deploy); C5 Livestream vẫn ngoài phạm vi (Sprint 18).*

*Cập nhật 2026-07-23 (chiều) — **Sprint 16 đóng phần code: D4 Hộp Thư + D5 Sự Kiện + D6 Phúc Lợi + D7 Đúc Bàn Cờ**: module backend chung `cchess-backend/src/economy/` (claim-once transaction + `wallet_tx` audit, admin CRUD + `economy:import` seed) — Mail (fan-out, read/claim/delete), Events (cửa sổ start/end, claim-once), Welfare (điểm danh chuỗi 7 ngày mốc nửa đêm VN, quà tân thủ, quà quay lại ≥7 ngày), Crafting (đốt nguyên liệu+coins→cosmetic). Flutter `presentation/economy/` 4 màn + `EconomyController` + Explore hub 4 tile sống + badge thư chưa đọc. **Bug fix quan trọng**: puzzle API `owns()` từng chiếm toàn bộ `/admin/*` đứng đầu chain `server.ts` → 403/404 cho `/admin/shop`, `/admin/community/feed` qua server production; đã thu hẹp về namespace riêng + regression test. Test: backend **206/206**, Flutter **434 pass**. Cùng ngày: deploy rules + seed lên `cchess-dev`, mở event window, seed shop catalog 13 item, QA Android (emulator Phone_3) **PASS toàn bộ** vòng lặp hub→event→craft→balo→điểm danh→mail; phát hiện 2 bug nhỏ và fix ngay trong ngày (mail badge cache autoDispose, onboarding không bền sau force-stop — root cause thật: `ProfileController.update()` return sớm khi state còn loading, đã sửa await `_initialLoad`). Test hardening thêm: backend **230/230**, Flutter **462/462**.*

*Cập nhật 2026-07-23 (tối) — **Review toàn luồng người dùng trên emulator (Phone_3), đóng P1 + P2**: build từ main `12a0800`. **P1 (5 commit `8094b3d..a768665`)**: `PopScope` xác nhận thoát khi đang chơi (system back lẫn app-bar back); app-bar avatar→profile, chuông→mail (badge `unreadMailCountProvider`), gear→settings đều nối route thật (trước đó chết); lobby online tự kết nối, URL editor debug chỉ hiện ở `kDebugMode`; lỗi mua hàng hiện ngay trong bottom sheet (trước đó snackbar bị che, vô hình); Trang Chủ dùng chung `DailyChallengeBanner` (widget mới) cho cả Home/Học Cờ/puzzle list + card điểm danh nối `welfareProvider` thật. **P2 (6 commit `cd307fe..bcf7fca`)**: rời ván-vs-bot = xử thua thật (persist + trừ ELO); khoá undo trong ván rated; hotseat không tính ELO; overlay nút xếp dọc; đếm nước tàn cục `(step+1)~/2`; route Giải Đấu→tournaments, Kỳ Phổ→history; camera tile "Sắp có"; grid huy chương thật + share app + help dialog + VIP "Sắp có" ở Hồ Sơ; dialog chính sách/điều khoản ở Settings; header Học Cờ + hoạt động gần đây dùng data thật (3 ván gần nhất, tap→replay); nhãn minh họa cho danh sách bạn bè; `RewardChips`/crafting tra tên đẹp catalog (`itemDisplayNamesProvider`); thống nhất đơn vị "xu"; backend `sendMail` nhận `dedupeKey` (doc id `dk_<key>`) chống trùng broadcast; luồng game/bot-select/lobby chuyển hẳn sang push + back pop về đúng nơi. **Root cause quan trọng đã sửa**: Duplicate GlobalKey exception = `AnimatedSwitcher` trong `AppShell` giữ lại subtree của tab cũ khi chuyển tab — bỏ hẳn, chuyển fade sang `CustomTransitionPage` của router; repro sau fix = 0. Verify từng fix trên emulator, Flutter 462/462 rồi 464/464 (thêm 2 test dedupe mail phía backend → 232/232). **Còn lại không sửa được từ code**: Google Sign-In `DEVELOPER_ERROR` trên emulator — cần thêm SHA-1 debug keystore vào Firebase console.*

*Cập nhật 2026-07-28 — **Polish UI thuần túy, không thuộc sprint nào trong kế hoạch gốc**: quân cờ vẽ lại bằng `_TiltedCylinderPiecePainter` (hiệu ứng gỗ sơn mài nghiêng, thay Container gradient tròn phẳng cũ), giữ nguyên các trạng thái `faceDown`/`selected`/`inCheck`/`lastMoveHighlight`; `pieceDiameter` tăng lên 0.90 trong `board_painter.dart`. Thêm bộ widget test riêng cho `ChessPieceWidget` + golden preview image. Commit `3258140`.*

*Cập nhật 2026-08-07 — **Rà soát tiến độ, không có code mới trong phiên này**: đối chiếu `git log` (HEAD `3258140`, `main` sạch + đồng bộ `origin/main`) và chạy lại trực tiếp `npm test` (backend, `cchess-backend/`) + `flutter test` (`cchess/`) để xác nhận số liệu thay vì tin vào con số cũ trong doc. Kết quả: backend **232 test, 231 pass / 1 fail** (flaky-timing ở `server.test.ts` M1/M4), Flutter **464 test, 463 pass / 1 fail** (flaky-timing ở `bot_engine_test.dart` best-effort hint). Cập nhật lại §1/§5/§6/§7 cho khớp các mốc đã push nhưng chưa được ghi vào doc: Sprint 16 D4–D7 (2026-07-23), đợt review toàn luồng người dùng P1+P2 (2026-07-23 tối, 11 commit dọn nút chết + dedupeKey mail + fix root cause Duplicate GlobalKey), và polish 3D piece painter (2026-07-28, ngoài khung sprint chính thức).*
