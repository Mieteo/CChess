...
*Tạo 2026-06-07 sau khi chốt phương án lai (offline minimax Dart + online Pikafish server-side). Cập nhật 2026-06-07: đã triển khai service engine/API/pool/cache/quota cơ bản, Dockerfile engine, Flutter abstraction/router/fallback và đã nối bot/replay controller vào `EngineRouter`. Cập nhật 2026-06-11: **nút Gợi ý in-game + attribution GPL trong Cài đặt đã xong** (Phase 4 chỉ còn màn AI Coach B3 chuyên biệt); test engine-service 6/6 nằm trong `npm test` 25/25 (xem Nhóm T9 của [`10_KE_HOACH_TEST.md`](10_KE_HOACH_TEST.md)). Cập nhật 2026-06-19: đã thêm `npm run engine:smoke` + workflow `engine-smoke` để smoke black-box `/health`, auth, best-move/cache, hint, analyze và quota opt-in; bước tiếp theo là chạy script với Pikafish thật (local Docker hoặc Render `cchess-engine`), deploy engine, quota/VIP bền vững.*
