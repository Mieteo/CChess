...
- ✅ **Định tuyến engine chuyển sang thang ELO liên tục — DONE 2026-06-26** (doc [13](13_KE_HOACH_ELO_BOT_LADDER.md) Phase 0–6): thay luồng cũ "card Đại Sư+ / `level=grandmaster` → remote" bằng `configForElo(targetElo)` → `EngineConfig{engine, depth, movetime, skill, uciElo, blunderRate}`; `EngineRouter` chọn remote khi `config.engine == remotePikafish`. **ElephantEye native (FFI Android) đã wire vào dải 1500–1900**, minimax dải 1000–1400, Pikafish dải 2000–2900+. Backend Pikafish nhận `elo`/`skill`, **cache key kèm strength** + reset sau search. **Còn lại: calibrate bảng số thủ công** (doc 13 §8) + build `.so` ElephantEye cho mọi ABI.
