...
1. ✅ **Quota/VIP bền vững (DONE 2026-06-23)** — `FirestoreQuotaStore` + `createFirestoreVipChecker` (Firestore-backed, reset theo ngày, VIP bypass, fail-open). Chi tiết §10.2.
2. 🟡 **FEN/UCI + H4 chất lượng** — ✅ fixture CI `fen_uci_compat.test.ts` (5 test) + cross-check `FIXED_POSITIONS` trong `engine_smoke.ts` đã khóa quy ước & đối chiếu nước Pikafish hợp lệ trên board ta; còn chạy smoke với Pikafish thật sau deploy + benchmark chất lượng H4. Chi tiết §10.2.
...
4. ✅ **AI Coach B3 (DONE 2026-06-23)** — `CoachAnalyzer` (pure) diễn giải `GameAnalysis` thành báo cáo theo giai đoạn khai/trung/tàn cuộc + nhận xét + đề xuất luyện tập; màn `AiCoachScreen` qua `EngineRouter.analyze` (remote→fallback local). Chi tiết §10.2.
...
- ✅ **AI Coach B3 — DONE 2026-06-23**: lớp diễn giải rule-based `CoachAnalyzer` ([coach_analyzer.dart](cchess/lib/core/chess_engine/ai/coach_analyzer.dart), pure, 6 unit test) chia nước của người chơi theo giai đoạn khai/trung/tàn cuộc (heuristic theo số ply), tính độ chính xác từng giai đoạn, chỉ ra giai đoạn yếu nhất/mạnh nhất, liệt kê nước "bước ngoặt" và sinh nhận xét + gợi ý luyện tập (định tuyến sang puzzle/khai cuộc/xem lại). Màn `AiCoachScreen` ([coach/](cchess/lib/presentation/coach/)) chạy qua `EngineRouter.analyze` (remote Pikafish → fallback minimax offline). Entry: ô **"AI Tư Vấn"** trong Học Cờ (`/ai-coach`, phân tích ván mới nhất) + nút 🧠 trên mỗi dòng Kỳ Phổ (`/ai-coach/:id`). Flutter `flutter test` 233/233, analyze sạch.
- 🟡 **Spike FEN/UCI với Pikafish thật**: (a) **fixture CI DONE 2026-06-23** — [fen_uci_compat.test.ts](cchess-backend/src/engine/fen_uci_compat.test.ts) khóa quy ước UCCI/Pikafish (file a-i, rank 0 = đáy Đỏ): round-trip FEN, codec UCI, mỏ neo tọa độ trên thế khởi đầu, áp `h2e2` ra đúng FEN, reject nước off-board (5 test, chạy không cần Pikafish). (b) **cross-check smoke DONE 2026-06-23** — `engine_smoke.ts` thêm bộ `FIXED_POSITIONS` (khởi đầu / sau pháo đầu / tàn cuộc Xe-Tướng): nước `best-move` Pikafish trả về phải **hợp lệ trên board của ta** (`isValidMove`), bắt ngay nếu lệch tọa độ. (c) **Còn lại:** chạy smoke này với service Pikafish thật sau deploy + đánh giá **chất lượng H4** (so nước gợi ý với kỳ vọng đại sư, tinh chỉnh movetime/depth).
...
- ✅ **VIP thật + quota bền vững — DONE 2026-06-23**: `FirestoreQuotaStore` ([firestore_quota.ts](cchess-backend/src/engine-service/firestore_quota.ts)) đếm hạn mức trong transaction tại `users/{uid}/engine_usage/{YYYY-MM-DD}` (có `expireAt` cho Firestore TTL), nên quota **sống sót qua restart/redeploy**. VIP đọc thật từ `users/{uid}.isVip` + `vipExpiresAt` qua `createFirestoreVipChecker` (cache 60s/uid). Cả hai **fail-open an toàn**: lỗi Firestore → quota tụt về in-memory cap, VIP tụt về free (không bao giờ phát free không giới hạn). `server.ts` mặc định dùng Firestore khi production (auth bật) trừ khi `ENGINE_QUOTA_BACKEND=memory`. Test: `firestore_quota.test.ts` 6 case (giới hạn + sống sót restart + reset theo ngày + VIP bypass + fallback lỗi). Backend `npm test` **146/146** (tính tới 2026-06-26). **TTL policy + deny-rule đã đưa vào code 2026-06-26** (`firestore.indexes.json` `fieldOverride` cho `engine_usage.expireAt` + `firestore.rules` deny client trên `engine_usage`). **Deploy:** ✅ rules + indexes đã lên **cchess-dev 2026-06-27** (`firebase deploy --only firestore:rules,firestore:indexes`); xác nhận TTL `engine_usage.expireAt` "Active" trong Console. **Còn lại:** deploy **prod** (`-P prod`).
- ✅ **Định tuyến engine chuyển sang thang ELO liên tục — DONE 2026-06-26, cơ chế Pikafish reworked 2026-06-30** (doc [13](13_KE_HOACH_ELO_BOT_LADDER.md) Phase 0–6 + §9): thay luồng cũ "card Đại Sư+ / `level=grandmaster` → remote" bằng `configForElo(targetElo)` → `EngineConfig{engine, depth, movetime, blunderRate}`; `EngineRouter` chọn remote khi `config.engine == remotePikafish`. **ElephantEye native (FFI Android) đã wire vào dải 1500–1900** (build .so đa-ABI xác nhận 2026-06-30), minimax dải 1000–1400, Pikafish dải 2000–2900+. Backend Pikafish nhận `blunderRate` (không phải `elo`/`skill` — bản Pikafish chính thức không hỗ trợ `UCI_LimitStrength`/`UCI_Elo`/`Skill Level`, xác nhận bằng binary thật), cache **bypass khi blunder active** (không đưa vào key). **Còn lại: calibrate bảng `blunderRate` Pikafish + cài app lên thiết bị thật để xác nhận hết crash** (doc 13 §9).
